...
class module_extra extends module_base{
    ...
    public function process(){
        if('save_extra_default' == $_REQUEST['_process']){
            ...
            if(isset($_REQUEST['butt_del'])){
                // todo: find out what records are using this extra field
                // todo: promt the user if htey want to remove these extra fields as well
            ...
            // todo: rename any existing extra fields to this one.
    ...
    public static function get_defaults($owner_table=false) {
        ...
        $sql = "SELECT `extra_default_id`,`extra_key`, `order`, `display_type`, `owner_table`, `searchable` FROM `"._DB_PREFIX."extra_default` e $where ORDER BY e.`order` ASC";
        foreach(qa($sql) as $r){
