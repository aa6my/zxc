...
?><tr class="task_editting task_row_<?php echo $task_id;?>">
    ...
    <td nowrap="">
        <?php if($task_editable){ ?>
            <?php echo currency('<input type="text" name="job_task['.$task_id.']['.$hours_prefix.'amount]" value="'.($task_data[$hours_prefix.'amount'] != 0 ? number_out($task_data[$hours_prefix.'amount']) : number_out($task_data[$hours_prefix.'hours']*$job[$hours_prefix.'hourly_rate'])).'" id="'.$task_id.'taskamount" class="currency" tabindex="13">');?>
        <?php }else{ ?>
            <?php echo $task_data[$hours_prefix.'amount'] != 0 ? dollar($task_data[$hours_prefix.'amount'],true,$job['currency_id']) : dollar($task_data[$hours_prefix.'hours']*$job[$hours_prefix.'hourly_rate'],true,$job['currency_id']);?>
        <?php } ?>
    </td>
    <?php } ?>
