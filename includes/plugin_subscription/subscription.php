        ...
        if($full_finance_item && isset($full_finance_item['finance_id']) && (int)$full_finance_item['finance_id']>0){
            ...
            if($subscription){
                ...
                $new_finance = array(
                    ...
                    'description' => _l('Payment against invoice #%s on subscription %s',module_invoice::link_open($invoice_id,true),$subscription_name), // . var_export($full_finance_item,true),
            ...
            if($subscription['automatic_renew']){
                ...
                foreach($owners as $member){
                    ...
                    if($next_due_time <= strtotime(date('Y-m-d')) && !$next_due_time_invoice_created){
                        ...
                        if($debug) echo " - next subscription time is ".print_date($next_due_time)." so its time to generate an invoice!<br>\n";
