...
','File Comment: {FILE_NAME} {CUSTOMER_NAME}',array(
	...
	public static function link_open_email($file_id,$full=false){
        return self::link_generate($file_id,array('full'=>$full,'page'=>'file_admin_email'));
    }
		...
		if('plupload' == $_REQUEST['_process']){
            ...
            if(isset($_REQUEST['butt_del']) && self::can_i('delete','Files')){
                ...
                if(self::can_i('edit','Files') || self::can_i('create','Files')){
                    ...
                    if(isset($_POST['staff_ids_save']) && (int)$file_id > 0){
                        delete_from_db('file_user_rel',array('file_id'),array($file_id));
                    ...
                    if($file_changed){
                        $this->send_file_changed_notice($file_id, $new_file);
                    }// file changed
