...
//                            if(!$quote_data || $quote_data['quote_id']!=$task['quote_id'])continue;
    ...
    public static function get_replace_fields($quote_id,$quote_data=false){
        ...
        if(isset($quote_data['customer_id']) && $quote_data['customer_id']){
            $customer_data = module_customer::get_replace_fields($quote_data['customer_id']);
...
//        $data = array_merge($data,$customer_data,$website_data,$quote_data);
	...
	public function process(){
		...
		if(isset($_REQUEST['butt_del']) && $_REQUEST['butt_del'] && $_REQUEST['quote_id']){
            ...
            exit;
		}else if("save_quote_tasks_ajax" == $_REQUEST['_process']){

            // do everything via ajax. trickery!
            // dont bother saving the quote. it's already created.
            ...
            switch($result['status']){
                case 'created':
                    ...
                    if((int)$result['quote_task_id']>0){
                        ...
                        <script type="text/javascript">
                            ...
                            <?php if($quote_data['status']!=$new_status){ ?>parent.jQuery('#status').val('<?php echo $new_status;?>').change();<?php } ?>
                            <?php if($new_quote_data['date_approved']!=$quote_data['date_approved']){ ?>parent.jQuery('#date_approved').val('<?php echo print_date($new_quote_data['date_approved']);?>').change();<?php } ?>
                        </script>
                    <?php }else{
                        set_error('New task creation failed.');
                        ?>
                        <script type="text/javascript">
                            top.location.href = '<?php echo $this->link_open($_REQUEST['quote_id']);?>&added=true';
                        </script>
                    <?php
                    }
                    break;
                case 'deleted':
                    ...
                    ?>
                    <script type="text/javascript">
                        top.location.href = '<?php echo $this->link_open($_REQUEST['quote_id']);?>';
                        <?php if($quote_data['status']!=$new_status){ ?>parent.jQuery('#status').val('<?php echo $new_status;?>').change();<?php } ?>
                    </script>
                    <?php
                    break;
			...
			$quote_id = $this->save_quote($_REQUEST['quote_id'],$_POST);
...
//            $quote['total_amount_discounted'] = 0;
	...
	public static function save_quote($quote_id,$data){
        ...
        if($quote_id){
            ...
            $result = self::save_quote_tasks($quote_id,$data);
