...
<form action="" method="post" id="quote_form">
    ...
    // files
    ob_start();
    $files = module_file::get_files(array('quote_id'=>$quote['quote_id']),true);
        if(count($files) > 0){
            ?>
            <a href="<?php
		    ...
		    $fieldset_data['elements'][] = array(
			    ...
			    'fields' => array(
				    function () use ( $quote_id, &$quote ) {
					    ...
					    echo print_select_box( $quote_default_tasks, 'default_task_list_id', '', '', true, '', true );
					    ?>
    ...
    $form_actions = array(
        ...
        'elements' => array(
            ...
            array(
                'ignore' => !function_exists('convert_html2pdf'),
                'type' => 'submit',
                'name' => 'butt_print',
                'value' => _l('Print PDF'),
            ),
...
?>
                    ...
                    <?php if(module_config::c('quote_ajax_tasks',1)){ ?>
                        <iframe name="quote_task_ajax_submit" id="quote_task_ajax_submit" src="about:blank" style="display:none; width:0; height:0;" frameborder="0"></iframe>
                    <?php } ?>

                    <form action="" method="post" id="quote_task_form" <?php if(module_config::c('quote_ajax_tasks',1)){ ?> target="quote_task_ajax_submit"<?php } ?>>
                        <input type="hidden" name="_process" value="save_quote<?php if(module_config::c('quote_ajax_tasks',1)) echo '_tasks_ajax'; ?>" />
                        <input type="hidden" name="quote_id" value="<?php echo $quote_id; ?>" />
                        <input type="hidden" name="customer_id" value="<?php echo $quote['customer_id']; ?>" />
                ...
                if(count($jobs)){
                    ...
                    ?>
                        <div class="tableclass_form content">
                            ...
                            <p align="center">
                                <?php echo _l('This Quote has been converted into a Job: %s.',module_job::link_open($j['job_id'],true,$j)); ?><br/>
                            </p>
                            <?php } ?>
                        </div>
                    <?php
