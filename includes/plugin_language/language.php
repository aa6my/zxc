...
class module_language extends module_base{
	...
	public function process() {
		...
		if ( 'language_duplicate_remove' == $_REQUEST['_process'] && $_REQUEST['really'] == 'yep') {
            ...
            // delete all language words and translations
            $sql = "SELECT CAST(`word` as BINARY) AS `word`, COUNT(*) as cc FROM `"._DB_PREFIX."language_word`  GROUP BY `word` HAVING cc > 1";
            $res = qa($sql);
            foreach($res as $r){
                if($r['word'] && $r['cc'] > 1){
                    // remove duplicates.
                    $sql = "SELECT * FROM `"._DB_PREFIX."language_word` WHERE BINARY `word` = '".mysql_real_escape_string($r['word'])."' ";
                    $duplicates = qa($sql);
                    $first = false;
                    foreach($duplicates as $duplicate){
                        if($first === false)$first = $duplicate;
                        else if($first['language_word_id']){
                            // remove this one and replace any translations with the first one.
                            $sql = "DELETE FROM `"._DB_PREFIX."language_word` WHERE language_word_id = '".(int)$duplicate['language_word_id']."' LIMIT 1";
                            query($sql);

                            $sql = "UPDATE `"._DB_PREFIX."language_translation` SET language_word_id = '".(int)$first['language_word_id']."' WHERE language_word_id = '".(int)$duplicate['language_word_id']."'";
                            query($sql);
	...
	public static function set_ui_language($language_code){
		...
		if(self::is_language_db_enabled()) {
			...
			if ( count( $labels ) ) {
				$sql      = "SELECT `word` FROM `" . _DB_PREFIX . "language_word` ";
				$res      = query( $sql );
				$db_words = array();
			...
			// now we look in the database for any language translations
			// these override the ones from the files above.
			...
			$sql .= " LEFT JOIN ";
			$sql .= " `" . _DB_PREFIX . "language` l ON lt.language_id = l.language_id  ";
			$sql .= " WHERE lt.language_id IS NULL OR l.language_code = '" . mysql_real_escape_string( $language_code ) . "'";
			$res = query( $sql );
	...
	public static function missing_word($text, $url='', $translation = false){
		...
		if(self::is_language_db_enabled()) {
			...
			$sql = "SELECT * FROM `"._DB_PREFIX."language_word` WHERE BINARY `word` = '".mysql_real_escape_string($text)."'";
			$res = qa1($sql);
			if(!$res || !$res['language_word_id']) {
		...
		if(!_DEMO_MODE && isset($extra_options['language_id']) && $extra_options['language_id'] > 0 && count($data)>1){
			foreach($data as $row){
				if(isset($row['word']) && strlen($row['word']) && isset($row['translation']) && strlen($row['translation'])){
					...
					$sql .= " FROM ";
					$sql .= " `"._DB_PREFIX."language_word` lw ";
					$sql .= " WHERE BINARY lw.`word` = '".mysql_real_escape_string($row['word'])."'";
					$res = qa1($sql);
					...
					if($res && $res['language_word_id']){
						$language_word_id = $res['language_word_id'];
					}else{
	...
	public static function l($text){
		...
		if(isset($labels[$text]) && strlen(trim($labels[$text]))){
			...
			//echo $text." is missing"; print_r($labels);exit;
